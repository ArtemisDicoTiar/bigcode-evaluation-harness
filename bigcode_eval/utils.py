...
from bigcode_eval.deepseek_utils import extract_generation_code
...
def complete_chat_code(
    ...
    **gen_kwargs,
    ...
    extracted_code_gens = defaultdict(list)
    for step, batch in tqdm(
            enumerate(dataloader),
            total=math.ceil(
                n_tasks * dataloader.dataset.n_copies / accelerator.num_processes
        ...
        for task_id, question, language in zip(task_ids, questions, languages):
            generated_codes = code_gens[task_id]
            for generated_code in generated_codes:
                ...
                extracted_example = extract_generation_code(example, lang_code=language)

                extracted_code_gens[task_id].append(extracted_example)

    generations.extend(extracted_code_gens)
    return generations
